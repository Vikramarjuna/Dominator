...
The protocol-neutral `lib/ratelimit` package provides three token-bucket limiters, in addition to the existing per-user-method concurrency limiter in `lib/srpc/serverutil`, each guarding against a distinct failure mode:
...
```json
{
    "Global": {"RequestsPerSecond": 10000, "Burst": 20000},
    "PerMethod": {
        "CreateVm":  {"RequestsPerSecond": 20, "Burst": 40},
        "DestroyVm": {"RequestsPerSecond": 20, "Burst": 40}
    },
    "PerUserPerMethod": {
        "Default": {"RequestsPerSecond": 1, "Burst": 5},
        "Overrides": {
            "ListVMs":    {"RequestsPerSecond": 10, "Burst": 20},
            "GetMdbData": {"RequestsPerSecond": 10, "Burst": 20}
        }
    }
}
```

A single `ratelimit.Limiter` instance is shared across protocols within a server: the SRPC server attaches it via the `srpc.MethodBlocker` adapter in `lib/srpc/serverutil` (`RateLimiterBlocker`), while the gRPC interceptor and REST handler call its `Allow` method directly. The shared instance ensures the buckets and denial counters are common to all three protocols, so a client cannot bypass a quota by switching transports.
...
A counter registered at `ratelimit/<sub-directory>/<method>/<limit-type>/<protocol>` records requests rejected by a rate limiter, where the limit type is one of `global`, `per_method` or `per_user_per_method`. Individual denials are also logged at debug level with the user identity, the method and the limit type, to support investigation of specific clients without inflating metric cardinality.
